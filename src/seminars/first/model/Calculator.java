    ...
    public static double calculatingDiscount(double purchaseAmount, int discountAmount) {
        ...
        if (purchaseAmount >= 0) {
            if (discountAmount >= 0 && discountAmount<=100) {
                resamount = purchaseAmount - (purchaseAmount * ((double) discountAmount / 100));
            } else throw new ArithmeticException("Процент скидки может быть только от 0 до 100");
        } else throw new ArithmeticException("Сумма покупки не может быть меньше 0");
        return  resamount;
    }
}